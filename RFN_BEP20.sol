...
contract BEP20RFN is ERC20 {
  ...
  constructor(uint256 fee_rate, uint256 unlocking_RFN, address new_staff, uint256 new_staff_unlocked_RFN,
    ...
    _unlocked_RFN_total = unlocking_RFN;
    _fee_income = 0;
    staff_add(new_staff, new_staff_unlocked_RFN);
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    if (msg.sender == _owner) {
      require(balanceOf(_owner) - (_unlocked_RFN_total - staff_quota_total()) >= amount, "sendable RFN is not enough");
    }
    else {
  ...
  function staff_add(address new_staff, uint256 new_staff_unlocked_RFN) onlyOwner public returns (bool) {
    ...
    transfer(new_staff, new_staff_unlocked_RFN);
    arr_staff.push(bridge_staff(new_staff, new_staff_unlocked_RFN));
    return true;
  }
  ...
  function staff_quota_add(address staff, uint256 increased) onlyOwner external {
    ...
    require(_unlocked_RFN_total - staff_quota_total() > increased, "you can add within your unlocked_RFN");
    for (uint256 i=0; i<arr_staff.length; i++) {
  ...
  function staff_quota_minus(uint256 decreased) onlyStaff external {
    ...
    require(quota >= decreased, "you can minus within your unlocked_RFN");
    for (uint256 i=0; i<arr_staff.length; i++) {
      if (arr_staff[i].user == msg.sender) {
  ...
  function unlocked_RFN_total() external view returns (uint256) {
    return _unlocked_RFN_total;
  }
  
  event evt_unlocked_RFN_total_add(uint256 _unlocked_RFN_total);
  function unlocked_RFN_total_add(uint256 amount) onlyOwner external {
    require((balanceOf(_owner) - _unlocked_RFN_total) >= amount, "unlockable RFN is not enough");
    _unlocked_RFN_total += amount;
    emit evt_unlocked_RFN_total_add(_unlocked_RFN_total);
  }
  
  event evt_unlocked_RFN_total_minus(uint256 _unlocked_RFN_total);
  function unlocked_RFN_total_minus(uint256 amount) onlyOwner external  {
    require((_unlocked_RFN_total - staff_quota_total()) >= amount, "unlockable RFN is not enough");
    _unlocked_RFN_total -= amount;
    emit evt_unlocked_RFN_total_add(_unlocked_RFN_total);
  }
    ...
    for (uint256 i=0; i<len; i++) {
      if (arr_pegout_submit[del_id[i]].reg_date > 0) {
        if (arr_pegout_submit[del_id[i]].state == Submit_state.submit) {
          ...
          _unlocked_RFN_total += arr_pegout_submit[del_id[i]].amount;
      ...
      if (arr_pegin_reserve[id[i]].reg_date > 0) {
        if ( (arr_pegin_reserve[id[i]].user == msg.sender) && (arr_pegin_reserve[id[i]].state == Reserve_state.reserve) ) {
          ...
          if (result) {
            ...
            _unlocked_RFN_total += (arr_pegin_reserve[id[i]].amount + arr_pegin_reserve[id[i]].fee);
