...
import "@openzeppelin/contracts@v4.0.0/token/ERC20/ERC20.sol";

contract ERC20RFN is ERC20 {
  uint256 constant INIT_SUPPLY_RFN = 1000000000;  // 1,000,000,000
    
  address _owner;
  uint256 _locked_RFN_total;
  uint256 _fee_rate;
  ...
  constructor(uint256 fee_rate, uint256 locking_RFN, address new_staff, uint256 new_staff_locked_RFN,
                uint256 new_submit_daily_limit_total, uint256 new_submit_daily_limit_personal) ERC20("Rafen", "RFN") {
    _owner = msg.sender;
    _mint(_owner, (INIT_SUPPLY_RFN * (10 ** uint256(decimals()))));
    _locked_RFN_total = locking_RFN;
    staff_add(new_staff, new_staff_locked_RFN);
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    if (msg.sender == _owner) {
      require(balanceOf(_owner) - (_locked_RFN_total - staff_quota_total()) >= amount, "sendable RFN is not enough");
    }
    else {
      (bool is_staff, ) = staff_check(msg.sender);
      if (is_staff) {
        require(recipient == _owner, "staff can transfer RFN to the owner only");
      }
      else {
        (is_staff, ) = staff_check(recipient);
        require(!is_staff, "you can't transfer RFN to the staff");
      }
    }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
    if (sender == _owner) {
      ...
      (bool is_staff, uint256 quota) = staff_check(msg.sender);
      if (is_staff) {
        require(quota >= amount, "staff can transferFrom RFN within quota");
      }
    }
  ...
  function staff_add(address new_staff, uint256 new_staff_locked_RFN) onlyOwner public returns (bool) {
    ...
    transfer(new_staff, new_staff_locked_RFN);
    arr_staff.push(bridge_staff(new_staff, new_staff_locked_RFN));
    return true;
  }
  ...
  function staff_quota_add(address staff, uint256 increased) onlyOwner external {
    ...
    require(_locked_RFN_total - staff_quota_total() > increased, "you can add within your locked_RFN");
    for (uint256 i=0; i<arr_staff.length; i++) {
  ...
  function staff_quota_minus(uint256 decreased) onlyStaff external {
    ...
    require(quota >= decreased, "you can minus within your locked_RFN");
    for (uint256 i=0; i<arr_staff.length; i++) {
      if (arr_staff[i].user == msg.sender) {
  ...
  function locked_RFN_total() external view returns (uint256) {
    return _locked_RFN_total;
  }
  
  event evt_locked_RFN_total_add(uint256 _locked_RFN_total);
  function locked_RFN_total_add(uint256 amount) onlyOwner external {
    require((balanceOf(_owner) - _locked_RFN_total) >= amount, "lockable RFN is not enough");
    _locked_RFN_total += amount;
    emit evt_locked_RFN_total_add(_locked_RFN_total);
  }
  
  event evt_locked_RFN_total_minus(uint256 _locked_RFN_total);
  function locked_RFN_total_minus(uint256 amount) onlyOwner external {
    require((_locked_RFN_total - staff_quota_total()) >= amount, "lockable RFN is not enough");
    _locked_RFN_total -= amount;
    emit evt_locked_RFN_total_minus(_locked_RFN_total);
  }
  ...
  function pegin_submit(uint256 amount) external {
    ...
    _locked_RFN_total += (amount + calc_fee);
    ...
    for (uint256 i=0; i<len; i++) {
      if (arr_pegin_submit[del_id[i]].reg_date > 0) {
        if (arr_pegin_submit[del_id[i]].state == Submit_state.submit) {
          ...
          _locked_RFN_total -= (arr_pegin_submit[del_id[i]].amount + arr_pegin_submit[del_id[i]].fee);
  ...
  function pegout_reserve(uint256[] memory reg_date, bytes32[] memory id, address[] memory user, uint256[] memory amount) onlyStaff external {
    ...
    require(quota >= total_amount, "your unlocked_RFN balance is not enough");
    for (uint256 i=0; i<len; i++) {
      ...
      if (arr_pegout_reserve[id[i]].reg_date > 0) {
        if ( (arr_pegout_reserve[id[i]].user == msg.sender) && (arr_pegout_reserve[id[i]].state == Reserve_state.reserve) ) {
          ...
          if (result) {
            ...
            _locked_RFN_total -= arr_pegout_reserve[id[i]].amount;
